...
export default function SiteHeader({ variant }: { variant: Variant }) {
  ...
  const [spacerH, setSpacerH] = useState(110);
  const [open, setOpen] = useState(false);

  // Measure header height and push page content down (fix overlap globally)
  useLayoutEffect(() => {
  ...
  return (
    <>
      <header className="fixed left-0 right-0 top-0 z-50">
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10 pt-3">
          <div
            ref={headerRef}
          ...
          >
            ...
            {/* Mobile nav panel */}
            {open ? (
              <div className="lg:hidden border-t border-[rgb(var(--border))] pb-3 px-4">
        ...
        <style jsx global>{`
          ...
          .btnPrimary {
            ...
            box-shadow: 0 14px 40px rgb(var(--accent) / 0.2);
          }
          .btnPrimary:hover {
            transform: translateY(-1px);
      ...
      {/* Spacer: fixes overlap everywhere */}
